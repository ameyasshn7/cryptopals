...
def _bytes_xor(a: bytes, b: bytes,quiet = True, check_lens = False):
    if not quiet:
        print(a,"^",b)
    if check_lens and len(a) != len(b):
        raise ValueError('Byte lengths do not match')
...
def bytes_xor(*args: bytes, quiet=True, check_lens=False):
    ...
    for arg in args[1:]:
        result = _bytes_xor(a,b,quiet=quiet,check_lens=check_lens)
